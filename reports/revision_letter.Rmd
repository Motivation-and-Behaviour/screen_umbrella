...
We note that this increases the size of the table, and we have removed it from the text of the manuscript and instead included it as a seperate file.
...
We agree that the use of causal langauge from an evidence-base that is mostly cross-sectional would be misleading.
...
A summary of an entire field will, out of necessasity, be more broad that a typical systematic review.
...
Manual inspection of the reviews which did not contribute any effects suggests that this was sucessful - older reviews tended to only be included if they focused on the general population while a newer review focused on a sub-population.
...
\WorkInProgress
\todo{Havea added to table 1. Should we indicate on forest plots?}
...
We initially intended to include the quality assessment in Table 1, but felt it made the table too large and moved it to a seperate table.
It seems we inadvertently did not upload the seperate file when submitting the review.
...
1. \todo{I don't really want to add this - I think tables of counts are dumb. Thoughts?}
2. \todo{Yes, we should not have used the term "cases". Clear this up}
...
Some reviews presented multiple effect sizes, and once the information to describe each effect was added (e.g., the specifc outcome, exposure, age group, and moderator), the table became overwhelming.
We have included all of this information in the supplementary file.
...
\WorkInProgress
\todo{I think this is misleading. It differs by field, outcome, etc.}
...
1. \todo{Argue not subjective (pre-specified, objective), and we say in limitations that others might choose other options}
2. \todo{Agree - we pre-specified our approaching thinking that it balanced the need to be conservative with the need to be inclusive. But, many effects fail this (fairly low) bar. Add more on this to the discussion.}
...
We did not retrive data from the primary studies when the data were not provided in the paper or by contacting the author.
...
\todo{I kind of want to say "also those reviews are probably bad anyway, because it's not hard to put data in forest plots", but I'm not sure there's a professional way to say that.}
...
We note that while limiting to certain study designs may improve the quality of the evidence, we are simply summarising what previous reviews have done.
that is, if previous reviews have not limited their evidence to certain designs (or moderated by design), we do not have that information to present.
We also found that there were not a lot of reviews which examined the same exposure and outcome, but differed on choice of study design.
...
That is, there are not a lot of instances where the same exposure and outcome are examined in different study designs.

\WorkInProgress
\todo{include study design in the forest plots?}
...
We removed it because it made the table very large, but did not include the new file with the seperate table.
We have now included the quality assessment in Table 1.
...
We have included information about the sample in Table 1.
...
\todo{should 'clinical pops' be indicated in the forest plots?}
