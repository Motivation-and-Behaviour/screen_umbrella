...
manuscript <- revise::read_manuscript("reports/manuscript.md", PDF = TRUE)
```
...
Please see response to Reviewer 1, \Comment{causal_lang}.
...
We have removed the word "plausible", as we agree that makes the sentence less clear and this phrasing is more aligned with the eligibility critiera.
...
Per /comment{outcomes} above, we did not pre-specify outcomes and feel it would be inappropriate to revise our inclusion criteria.
...
This information is available here (not that the updated search increased the number of reviews):

`r revise::get_revision("r2_23")`
...
table and figure captions, variable dictionary…)
```
