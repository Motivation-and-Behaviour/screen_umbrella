...
We appreciate the feedback that you and the reviewers have provided. In the following itemised list we respond to each comment point-by-point.
...
The reviewer raised some important concerns, especially regarding the interpretability of odds ratios which we have decided should not have been included (see \comment{r2_16}).
...
We agree with the reviewer that the term 'screen time' is problematic as it fails to acknowledge the diversity of experiences screens provide.
Indeed, our motivation for conducting this review was to counter this narrative in the literature.
To do this, it was necessary to invoke the concept of 'screen time'.
As our results show, the influence of screen time on children is less a function of a physical screen, and more a function of the activity that the screen provides.
...
We also note that in reviewing the manuscript, we sometimes used the term 'screen time' and in other places used 'screen use'.
For the sake of being consistent, we have changed these to all be 'screen use' (which we think is more consistent with what is meant), and we also use this term for the rest of this letter.
This is just a stylistic change---we don't think that changing between 'time' and 'use' is what the reviewer means.
...
We acknowledge that the field contains a range of different effects with different magnitudes and directions.
We want to make clear what trade-offs are involved in any guidelines around screen use.
...
As a result, we think casting a wide net and using a meta-review methodology is one of the few ways of pragmatically outlining and quantifying the trade-offs.
As Cochrane says, "Overviews are an increasingly popular form of evidence synthesis, as they aim to provide 'user-friendly' summaries of the breadth of research relevant to a decision without decision makers needing to assimilate the results of multiple systematic reviews themselves (https://training.cochrane.org/handbook/current/chapter-v).
...
The assumption was that the largest review would be the most recent, and that newer reviews would contain the studies located in previous reviews. 
We inspected the reviews that did not contribute any effects.
This inspection suggested our assumptions were correct: older reviews tended to only be included if they focused on the general population while a newer review focused on a specific sub-population.
...
A strength of our study is that it points out issues with how research has been conducted in this field, which we hope will make future research more ambitious.
...
We have carefully reread the manuscript and removed any remaining unjustified causal language.
...
We agree that it would be useful to break down findings by age.
However, for us to accomplish this we need meta-analyses which examine the developmental span for each exposure and outcome combination.
While the addition of further reviews based on the reviewers' and editor's suggestion to update the search helped address this limiation (ewer papers were more likely to provide moderation by age group, and new reviews targeting specific age groups have been added), there were still too few age-based comparisons to make strong claims.
This is because many reviews target behaviours or outcomes which are more common in one age group (e.g., sexting in adolescents, or developmental outcome in young children).
For the effects where there were multiple age groups (e.g., body composition), differences tended to be small. 

We have included some additional information on this in the *Discussion* section:
...
The reviewer was pointing to the following quote from the *Lancet*: "Our understanding of the benefits, harms, and risks of our rapidly changing digital landscape is sorely lacking.".
It is not that the evidence is lacking, per se, but that our *understanding* is weak.
Our understanding is lacking partly because benefits of screen use (as seen in some educational literature) and reviewed separately from harms.
Our review aims to address this (see also Reviewer 1, \comment{r1_2}).
We think synthesising the trade-offs in one review can help us understand when screen use might be associated with good/poor outcomes, helping to better understand why.

We have added this additional context to the *Introduction*:
...
We have revised the introduction with new papers, with a particular focus on other umbrella reviews. See the below for an example of the changes:
...
Screen use is associated with a range of widespread outcomes.
Many reviews focus on a single exposure outcome pairing and miss the trade-offs that are inherent in focusing on just one area (see Reviewer 1, \comment{r1_2} for further discussion).
By focusing on a broad range of outcomes (health and non-health), without trying to *a-priori* predict what areas should be investigated, we simultaneously quantify and compare these trade-offs.
...
Guidelines are important and useful, but they should reflect both the most recent and the most comprehensive evidence.
Many of the previous reviews used to justify guidelines focused on narrow ranges of outcomes (e.g., the impact on adiposity).
We think this is a mistake---you cannot make an informed decision about an action if you only examine the risks, you must weigh these against any potential benefits.
We have included a discussion of this in the *Discussion* section:
...
We appreciate it is not a direct contrast to compare overall limits of screen use against deleterious effects of screen use over 3-4 hours. As a result, we have removed the words 'in contrast':
...
As mentioned in other author responses, we have made a stronger effort to organise the review toward a cohesive purpose.
...
We have tried not to preempt the results section in the introduction, but do now refer to some expected outcomes.
For example:
...
We intended to indicate that in order to make nuanced recommendations, you must have all of the information regarding benefits and risks.
...
As per \comment{r2_10} above, we did not pre-specify outcomes and do not want to mislead readers on the process.
We have indicated some anticiapted outcomes, however.
...
We agree that the relationship between some outcomes and screen use may not always be linear.
...
We agree this could be confusing to the reader.
The reason that we had inclusion/exclusion criteria across two sections is that we have separate criteria for reviews, and for the effect sizes within reviews.
The first section outlines the inclusion criteria for including a *review*, whereas the subsequent sections focus on reasons for excluding an *effect size* from the review.
...
We agree our phrasing here was unclear. We meant that we intended to narratively summarise systematic reviews, as well as meta-analytic studies. We have changed this phrase to:

`r get_revision("r2_20")`
...
It is true that harmonising effect sizes does not perfectly retain the original information.
Different measures are able to capture different aspects of the underlying phenomenon.
However, it is better to harmonise for the purposes of crude comparisons than to try and subjectively compare completely different classes of effect sizes. We make this point in the limitations:

`r get_revision("r2_16")`

In this study we had to choose between subjectively comparing various classes of effect sizes, or attempt to harmonise all effects so quantitative comparisons could easily be made. We elected to harmise the effect sizes so that we could better illustrate differences between studies. However, harmonisation is imperfect, and so to better understand the exact effect from meta-analyses we refer readers back to the original meta-analyses.
...
We appreciate we may have not explained this clearly.
We have brought our methods into line with other reviews.
In our protocol, we wrote that $P < 0.001$ would be *one of* the criteria for indicating strong evidence (alongside other metrics, such as sample size and heterogeneity).
We realised that this would restrict our 'strong' evidence to only statistically significant effects.
But using this criteria meant that we could never classify non-significant findings as strong evidence regardless of their precision, which is clearly a mistake.
...
1. We have added a table to summarise the effects (including some information on the reviews these effects were drawn from) as Supplementary File 9.
...
Most reviews presented multiple effect sizes, and once the information to describe each effect was added (e.g., the specific outcome, exposure, age group, and moderator), the table became overwhelming.
...
1. We agree that our reporting of the method could have been clearer, and have revised the text in this section to more clearly explain our method.
We designed this method to make it as objective as possible.
...
We can certainly see how others might use a different method for eliminating duplicate effects (and we state this in the discussion section), and we tested whether those methods would have led to different included reviews.
...
3. We understand the reviewer's concerns regarding the conversions of the effect sizes.
We appreciate that making conversions between effect sizes relies upon some assumptions.
We made the decision what it was better to include these studies rather than omit them, and better to convert the effects rather than presenting different tables for each effect size.
Still, we agree that readers should interpret the effect sizes carefully, and have included text to this effect in the *Discussion*:
...
We agree that the differences between the effects at different ages are important nuances.
We altered the analytical plan to make it more credible and reliable, but also want to make it so researchers can explore findings that we are less confident in
While we could offer a narrative synthesis of the 200+ effect sizes that did not meet our credibility criteria, we judged this was beyond the scope of this paper.
Doing so would involve some filtering and judgement, exacerbating the selection bias the reviewer points to.
As a result, we have presented those findings in supplementary materials for researchers and practitioners to explore the nuances relevant to them

```{asis r2_31}
Page 31 line 380-383: both guidelines mentioned recreational sedentary screen time. It is not in all instances. 
```

We agree both guidelines focus on merely reducing recreational screen use and have removed "in all instances".
Even within recreational screen use, however, our findings do not support guidelines which focus just on minimisation. 
Video games, for example, are a recreational form of screen use which appear to have some benefits for educational and cognitive outcomes. 
...
We agree that the supplementary materials are difficult to navigate.
We have added a codebook (as Supplementary File 3) as suggested.
...
AMSTAR-2 and NHLBI overlap on many items, and as AMSTAR-2 taks 20-30min per review we didn't feel that the extra length produced substantially more value.
We felt that the NHLBI was the appropriate standard for our review.
