...
all_high_unclear_txt <-
  paste0(
    ...
    if (all_high_unclear > 1 | all_high_unclear == 0) " meta-analyses" else " meta-analysis" # nolint
  )
...
There were `r n_edu_effect` unique effects associated with education outcomes, including general learning outcomes, literacy, numeracy, and science. We removed `r n_edu_noindiv` effects that did not provide individual study-level data, `r n_edu_samplesize` effects with samples < 1,000, and `r n_edu_faileggers` with a significant Egger's test or insufficient studies to conduct the test. No remaining studies showed evidence of excessive significance. Effects not meeting one or more of these standards are presented in [Supplementary File 4](https://drive.google.com/file/d/1wUotZRh-jvdBJh5dkstkMtvcz7DPn-vS/view?usp=sharing). The remaining `r n_edu_certain` met our criteria for statistical credibility and are described in Figure \@ref(fig:eduplot). These `r n_edu_certain` effects came from `r n_edu_unique` meta-analytic reviews analysing data from `r edu_summary$studies_n` empirical studies with `r format(edu_summary$total_n, big.mark=",")` individual participants. 
