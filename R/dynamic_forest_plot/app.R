...
q <- readRDS("clean_converted_data.RDS")
...
function_for_plotting <- function(q, outcome, arrange_by, font_size) {
    ...
    if(arrange_by != "Outcome"){
        part_q$plain_language_outcome <- paste("<i>",
                                               part_q$outcome_lvl_1,
                                               "</i><br>",
                                               part_q$plain_language_outcome,
                                               sep = "")
    }
    part_q$plain_language_exposure <- paste("<i>",
                                            part_q$plain_language_exposure,
                                            sep = "")
    part_q$plain_language_exposure <- gsub("\\: ",
                                           "</i><br>",
                                           part_q$plain_language_exposure)
    ...
    part_q$plain_language_outcome <- fct_expand(part_q$plain_language_outcome, "**Specific Outcome<br>(risks** & benefits**)**") %>%
        fct_relevel("**Specific Outcome<br>(risks** & benefits**)**")
    part_q$plain_language_outcome[last] <- "**Specific Outcome<br>(risks** & benefits**)**"
    ...
    } else if (arrange_by == "Effect Size"){
        part_q <- arrange(
            part_q,
            r
        )
    } else if (arrange_by == "Heterogeneity"){
        part_q <- arrange(
            part_q,
            i2
        )
    } else if (arrange_by == "K"){
        part_q <- arrange(
            part_q,
            k
        )
    } else if (arrange_by == "N"){
        part_q <- arrange(
            part_q,
            n
        )
    ...
    # Always put the legend at the top
    part_q$row_num <- fct_expand(part_q$row_num, "0") %>%
        fct_relevel("0")
    part_q$row_num[part_q$outcome_lvl_1 == "Outcome"] <- "0"
    ...
    p1 <- ggplot(
        ...
        {if(arrange_by == "Outcome")facet_grid(
        ...
        theme_mb() %+replace% theme(
            ...
            text = element_text(family = "Times", size = font_size*3)
        )
    p1
}
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            selectInput("outcome",
                        label = "Which outcome do you want to see?",
                        choices = c("Psychology", "Health Behaviour", "Physical Health", "Education")
            ),
            selectInput("sort",
                        label = "How do you want it sorted?",
                        choices = c("Outcome", "Exposure", "Alphabetically", "Effect Size", "Heterogeneity", "K", "N")
            ),
            sliderInput("size",
                        label = "Font size",
                        min = 1,
    ...
    output$distPlot <- renderPlot(
        {
            ...
            sort <- input$sort
            #sort <- "N"
            function_for_plotting(q, outcome, sort, input$size)
        },
        height = function() {
