...
make_desc_tables <- function(tables_df) {
  ...
  gt_table_main_original <-
    ...
    tab_header(html("<strong>Study Characteristics</strong>"),
      subtitle = "Review characteristics and quality assessment for studies providing unique effects" # nolint
    ) %>%
    cols_merge_range(
    ...
    cols_label(
      ...
      earliest_study_year = html(
        "Study Range<br><small>Earliest - Latest</small>"
      ),
      sample_ages = html(
  ...
  reviews_tables[["rob"]]$filename <- "Quality assessment table.pdf"
